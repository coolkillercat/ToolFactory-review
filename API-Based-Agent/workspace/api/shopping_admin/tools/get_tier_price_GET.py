...
def get_tier_price(sku=None, customerGroupId=None):
    """
    Get tier prices for a product by SKU and customer group ID.

    Args:
        sku (str): The SKU of the product
        customerGroupId (str): The customer group ID
    ...
    if isinstance(customerGroupId, tuple):
        customerGroupId = customerGroupId[1]

    api_url = f"http://ec2-3-129-135-45.us-east-2.compute.amazonaws.com:7780/rest/default/V1/products/{quote(sku, safe='')}/group-prices/{quote(str(customerGroupId), safe='')}/tiers"
...
if __name__ == '__main__':
    r = get_tier_price(sku='ABC123', customerGroupId='all')
    r_json = None
    try:
        r_json = r.json()
    except:
