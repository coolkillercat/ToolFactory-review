...
def update_downloadable_sample(sku=None):
    """
    Update downloadable sample for a product.
    ...
    """
    api_url = f'http://ec2-3-129-135-45.us-east-2.compute.amazonaws.com:7780/rest/default/V1/products/{quote(sku)}/downloadable-links/samples'

    # The error indicates 'sample' field is required
    payload = {
        'sample': {
            'title': 'Sample Title',
            'sort_order': 0,
...
if __name__ == '__main__':
    r = update_downloadable_sample(sku='ABC123')
    r_json = None
    try:
        r_json = r.json()
    except:
