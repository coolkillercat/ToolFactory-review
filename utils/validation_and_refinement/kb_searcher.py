...
def search_kb(query, param_keys, param_keys_emb, parameter_dict, description_keys, description_to_param_dict, description_keys_emb, model):
    """
    ...
    hits_description = [
        hit for hit in hits_description if hit['score'] > threshold]
    if not hits_param and not hits_description:
        return None
    elif not hits_param:
        return parameter_dict['['+description_to_param_dict[description_keys[hits_description[0]['corpus_id']]]+']']
    elif not hits_description:
