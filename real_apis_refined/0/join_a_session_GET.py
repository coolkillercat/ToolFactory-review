import requests
from urllib.parse import quote
                
def join_a_session(sessionid=None, name=None):
    api_url = f"https://connect-four.p.rapidapi.com/api/{quote(sessionid, safe='')}/{quote(name, safe='')}/join"
    querystring = {'sessionid': sessionid, 'name': name, }
    assert sessionid is not None, 'Missing required parameter: sessionid'
    assert name is not None, 'Missing required parameter: name'
    
    headers = {
        "X-RapidAPI-Key": "your_rapidapi_key",
        "X-RapidAPI-Host": "connect-four.p.rapidapi.com"
    }
    
    response = requests.get(url=api_url, params=querystring, headers=headers, timeout=50, verify=False)
    if response.status_code != 200:
        response2 = requests.get(url=api_url, headers=headers, timeout=50) # in case API can't handle redundant params
        response = response2
    return response
    # print(response.json())

if __name__ == '__main__':
    r = join_a_session(sessionid='''abc123''', name='''Player1''')
    r_json = None
    try:
        r_json = r.json()
    except:
        pass
    import json
    result_dict = dict()
    result_dict['status_code'] = r.status_code
    result_dict['text'] = r.text
    result_dict['json'] = r_json
    result_dict['content'] = r.content.decode("utf-8")
    print(json.dumps(result_dict, indent=4))