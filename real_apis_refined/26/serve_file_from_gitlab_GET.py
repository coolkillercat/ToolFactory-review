...
def serve_file_from_gitlab(user=None, repo=None, tag=None, file=None):
    api_url = f"https://cdn.statically.io/gl/{quote(user, safe='')}/{quote(repo, safe='')}/{quote(tag, safe='')}/{quote(file, safe='')}"
...
if __name__ == '__main__':
    r = serve_file_from_gitlab(user='''john_doe''', repo='''project-repo''', tag='''main''', file='''style.css''')
    r_json = None
    try:
        r_json = r.json()
    except:
