...
def returns_the_most_recently_added_machines_near_transmitted_lat_lon(lat, lon):
    ...
    if response.status_code != 200:
        ...
        print(response.text)
    # print(response.json())
returns_the_most_recently_added_machines_near_transmitted_lat_lon('45.5122', '-122.6587')
...
def suggest_a_new_location_to_add_to_the_map(location_name, location_machines):
    api_url = f"/api/v1/locations/suggest.json"
    payload = {'location_name': location_name, 'location_machines': location_machines, }
    response = requests.post(url=api_url, json=payload, timeout=50)
    if response.status_code != 200:
        print("--------------------------------------------")
        print(response.status_code)
        print(response.text)
    # print(response.json())
suggest_a_new_location_to_add_to_the_map('Pinball Palace', 'Machine1, Machine2')
...
def returns_the_closest_location_to_transmitted_lat_lon(lat, lon):
    ...
    if response.status_code != 200:
        ...
        print(response.text)
    # print(response.json())
returns_the_closest_location_to_transmitted_lat_lon('45.5122', '-122.658')
