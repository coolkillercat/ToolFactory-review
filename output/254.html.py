...
def query_payload_information(md5_hash, sha256_hash):
    ...
    if response.status_code != 200:
        ...
        print(response.text)
    # print(response.json())
query_payload_information('12c8aec5766ac3e6f26f2505e2f4a8f2', '35e304d10d53834e3e41035d12122773c9a4d183a24e03f980ad3e6b2ecde7fa')
...
def download_hourly_malware_batches():
    api_url = f"https://urlhaus-api.abuse.ch/v1/download/hourly/"
    querystring = {}
    response = requests.get(url=api_url, params=querystring, timeout=50)
    if response.status_code != 200:
        print("--------------------------------------------")
        ...
        print(response.text)
    # print(response.json())
download_hourly_malware_batches()

import requests
                
def download_daily_malware_batches():
    api_url = f"https://urlhaus-api.abuse.ch/v1/download/daily/"
    querystring = {}
    response = requests.get(url=api_url, params=querystring, timeout=50)
    if response.status_code != 200:
        print("--------------------------------------------")
        ...
        print(response.text)
    # print(response.json())
download_daily_malware_batches()
