...
def alerts(resource_id, alert_type, threshold_value):
    api_url = f"missing"
    payload = {'resource_id': resource_id, 'alert_type': alert_type, 'threshold_value': threshold_value, }
    response = requests.post(url=api_url, json=payload, timeout=50)
    if response.status_code != 200:
        print("--------------------------------------------")
        print(response.status_code)
        print(response.text)
    # print(response.json())
alerts('resource-123', 'threshold', 80)
